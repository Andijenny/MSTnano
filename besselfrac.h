#ifndef _BESSELFRAC_H_
#define _BESSELFRAC_H_
#include "nr3.h"
struct Bessel {
	static const Int NUSE1=7, NUSE2=8; //defination of static constant must be exclueded in struct/class.
	...
	inline void  besseljy(const Doub nu, const Doub x); //add inline tag to avoid mutipling defination
	inline void  besselik(const Doub nu, const Doub x); //
...
inline	void airy(const Doub x);
inline	Doub airy_ai(const Doub x);
inline	Doub airy_bi(const Doub x);

inline	void sphbes(const Int n, const Doub x);
inline	Doub sphbesj(const Int n, const Doub x);
inline	Doub sphbesy(const Int n, const Doub x);
