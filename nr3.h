...
public:
	...
	inline T max() const;
	inline T min() const;
	inline T sum() const; // added by Hao Wang, sum of a vector.
	void resize(int newn); // resize (contents not preserved)
	void resize(int newstart,int newend);//contents preserved in selected range
	void assign(int newn, const T &a); // resize and assign a constant value
	NRvector<T> operator+(const NRvector<T> & v2) const; // added by Hao Wang, define vector addition rules.
	NRvector<T> operator*(const NRvector<T> & v2) const; // vector mulitiplication, return a vector 
	NRvector<T> operator*(const T & n2 ) const; // vector mulitiplied by anumber,  return a vector
	NRvector<T> operator/(const T & n2 ) const; // vector mulitiplied by anumber,  return a vector
	friend NRvector<T> operator*(const T & n2,const NRvector<T> & v2)
		{return v2*n2;} // in general n2*v2 is equivalant to v2*n2
	~NRvector();
};
...
template <class T>
inline T NRvector<T>::max() const
{
	T t=v[0];
	for(int i=1;i<nn;i++)t=(t>v[i]?t:v[i]);
	return t;
}//added by Hao Wang

template <class T>
inline T NRvector<T>::min() const
{
	T t=v[0];
	for(int i=1;i<nn;i++)t=(t>v[i]?v[i]:t);
	return t;
}//added by Hao Wang
...
template <class T>
void NRvector<T>::resize(int newstart, int newend)
{
	if (newstart<0 || newend>=nn || newstart>newend){
		cout << newstart <<"-"<<newend<<endl;
		throw("exceed vector index!");}
	else{
		int newn;
		newn=abs(newend-newstart)+1;
		NRvector<T> vn(newn,0.0);
		for(int i=0;i<vn.size();i++)vn[i]=v[newstart+i];
//		return vn;
		nn = newn;
		if (v != NULL) delete[] (v);
		v=new T[nn];
		for(int i=0;i<nn;i++)v[i]=vn[i];
	}
}//added by Hao Wang
...
template <class T>
NRvector<T> NRvector<T>::operator+(const NRvector<T> & v2) const
{
	if (v2.size() != nn){
		throw("different sizes of two  added vectors");
		exit(0);
	}
	NRvector vsum(nn,0.0);
	for(int i=0;i<nn;i++) vsum[i]=v[i]+v2[i];
	return vsum;
}// vector addition ---added by Hao Wang

template <class T>
inline T NRvector<T>::sum() const
{	
	if(v == NULL || nn==0){
		throw("empty vector!");
		return 0;
	...
	T sum1=(T)0.0;
	for(int i=0;i<nn;i++) sum1=sum1+v[i];
	return sum1;
}// added by Hao Wang, sum of elements in vector..
template<class T>
NRvector<T> NRvector<T>::operator*(const NRvector<T> & v2) const 
{
	if (v2.size() != nn){
	...
	NRvector vpd(nn,0.0);
	for(int i=0;i<nn;i++) vpd[i]=v[i]*v2[i];
	return vpd;
}// added by Hao Wang, vector mulitiplication, return a vector 
	

template<class T>
NRvector<T> NRvector<T>::operator*(const T & n2) const 
{
	NRvector vn(nn,0.0);
	for(int i=0;i<nn;i++) vn[i]=n2*v[i];
	return vn;
}// added by Hao Wang, vector mulitiplied by a number 
...
{
	NRvector vn(nn,0.0);
	for(int i=0;i<nn;i++) vn[i]=v[i]/n2;
	return vn;
}// added by Hao Wang, vector divided by a number 
